...
class ModelGeneratorTest extends PHPUnit_Framework_TestCase
{
	...
	public function testCanGenerateModelUsingTemplate()
	{
		...
		$file->shouldReceive('put')->once()->with(app_path() . '/models/Foo.php', file_get_contents(__DIR__ . '/stubs/model.txt'));
	...
	public function testCanGenerateModelUsingCustomTemplateAndNoFields()
	{
		...
		$cache->shouldReceive('getFields')->once()->andReturn(false);

		$file->shouldReceive('put')->once()->with(app_path() . '/models/Foo.php', file_get_contents(__DIR__ . '/stubs/scaffold/model-no-fields.txt'));
	...
	public function testCanGenerateModelUsingCustomTemplateAndFields()
	{
		...
		$cache->shouldReceive('getFields')->once()->andReturn([
			                                                      'title' => 'string',
			                                                      'age'   => 'integer'
		                                                      ]);
