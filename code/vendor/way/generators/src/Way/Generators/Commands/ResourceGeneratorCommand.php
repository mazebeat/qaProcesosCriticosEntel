...
class ResourceGeneratorCommand extends Command
{
	...
	public function fire()
	{
		...
		// All done!
		$this->info(sprintf("All done! Don't forget to add '%s` to %s." . PHP_EOL, "Route::resource('{$this->getTableName($resource)}', '{$this->getControllerName($resource)}');", "app/routes.php"));
	...
	{
		return [['resource', InputArgument::REQUIRED, 'Singular resource name']];
	}
	...
	{
		return [['fields', null, InputOption::VALUE_OPTIONAL, 'Fields for the migration']];
	}
